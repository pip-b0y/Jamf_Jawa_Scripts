...
#To then trigger this script. The DEVICE WILL BE WIPED IN A  3 miniute window. Testing made easy!
...
curl -sk --header "authorization: Basic ${token}" ${jssurl}/JSSResource/mobiledevicecommands/command/EraseDevice/id/${id} -X POST
done
rm /tmp/*.xml
