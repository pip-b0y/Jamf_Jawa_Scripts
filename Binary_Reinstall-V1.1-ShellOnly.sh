#!/bin/bash
#Version 1.1 (shell only)
#By Hayden
#Reinstall the Jamf Binary based off Device ID
#Notes:
# - removed the dependancy of needing Python to pull out the token from the Auth 
...
api_token_raw=$(curl -X POST ${jssurl}/api/v1/auth/token -H "accept: application/json" -H "Authorization: Basic ${token_raw}")
